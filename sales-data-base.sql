...
SELECT EMPLOYEE_NUMBER, 
		FIRST_NAME, 
		LAST_NAME, 
		DATEDIFF(YEAR, BIRTH_DATE, '2025-01-09') -                       ---minus 1 from DATEDIFF provided employee is yet to celebrate birthday for the year
		CASE
			WHEN (MONTH(BIRTH_DATE) > MONTH('2025-01-09')) OR  
				 (MONTH(BIRTH_DATE) = MONTH('2025-01-09') AND DAY(BIRTH_DATE) < DAY('2025-01-09'))
				THEN 1																			---# SOLVED
				ELSE 0
		END AS AGE, 
		SUM (SALARY + COALESCE(COMMISSION,0)) AS SENIORITY		--- This column first turns NULL commision column to 0 before adding to salary					
FROM EMPLOYEES 
WHERE TITLE = 'Mr.' OR TITLE = 'Dr.' 
GROUP BY EMPLOYEE_NUMBER, FIRST_NAME, LAST_NAME, BIRTH_DATE 
HAVING SUM (SALARY + COALESCE(COMMISSION,0)) >= 8000
ORDER BY SUM (SALARY + COALESCE(COMMISSION,0)) DESC
SELECT * FROM EMPLOYEES



--:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
--- 4. Display products that meet the following criteria: (C1) quantity is packaged in bottle(s), (C2) the third character in the product name is 't' or 'T', (C3) supplied by suppliers 1, 2, or 3, (C4) unit price ranges between 70 and 200, and (C5) units ordered are specified (not null). 
...
WHERE 
	...
	AND PRODUCT_NAME LIKE '__T%' or PRODUCT_NAME LIKE '__t%'						---# SOLVED
...
SELECT ORD.ORDER_NUMBER,
       ORDER_DETAILS.UNIT_PRICE * ORDER_DETAILS.QUANTITY AS ORDER_AMOUNT,
       CASE
			WHEN ORDER_DETAILS.UNIT_PRICE * ORDER_DETAILS.QUANTITY <= 2000 THEN 0
			WHEN ORDER_DETAILS.UNIT_PRICE * ORDER_DETAILS.QUANTITY BETWEEN 2001 AND 10000 THEN 0.05
			WHEN ORDER_DETAILS.UNIT_PRICE * ORDER_DETAILS.QUANTITY BETWEEN 10001 AND 40000 THEN 0.1
			WHEN ORDER_DETAILS.UNIT_PRICE * ORDER_DETAILS.QUANTITY BETWEEN 40001 AND 80000 THEN 0.15
			WHEN ORDER_DETAILS.UNIT_PRICE * ORDER_DETAILS.QUANTITY > 80000 THEN 0.20
	   END AS NEW_DISCOUNT_RATE,

	   CASE
			WHEN ORD.ORDER_NUMBER > 10999 THEN 'Apply NEW discount rate'
			WHEN ORD.ORDER_NUMBER BETWEEN 10000 AND 10999 THEN 'Apply OLD discount rate'
	   END AS APPLICATION_NOTE

FROM ORDERS AS ORD
JOIN ORDER_DETAILS
ON ORDER_DETAILS.ORDER_NUMBER = ORD.ORDER_NUMBER
WHERE ORD.ORDER_NUMBER >= 10998 AND ORD.ORDER_NUMBER <= 11003;
...
FROM SUPPLIERS S
JOIN PRODUCTS P
ON P.SUPPLIER_NUMBER = S.SUPPLIER_NUMBER												---# SOLVED
...
SELECT C.CUSTOMER_CODE,
	   COUNT(C.CUSTOMER_CODE) AS COUNTS 
FROM CUSTOMERS AS C
FULL OUTER JOIN ORDERS AS ORD
ON C.CUSTOMER_CODE = ORD.CUSTOMER_CODE
FULL OUTER JOIN ORDER_DETAILS AS OD																---- # SOLVED WITH NO OUTPUT
ON OD.ORDER_NUMBER = ORD.ORDER_NUMBER
FULL OUTER JOIN PRODUCTS AS P
ON P.PRODUCT_REF = OD.PRODUCT_REF
WHERE C.CITY = 'Berlin' AND 
	  P.CATEGORY_CODE = 3
GROUP BY C.CUSTOMER_CODE
HAVING COUNT(C.CUSTOMER_CODE) <= 1
...
SELECT CUS.CUSTOMER_CODE, 
	   CUS.COMPANY, 
	   CUS.PHONE,
	   COUNT(DATENAME(WEEKDAY, ORDER_DATE)) AS 'TOTAL AMOUNT',
	   CUS.COUNTRY
FROM CUSTOMERS AS CUS
FULL OUTER JOIN ORDERS AS ORD
ON CUS.CUSTOMER_CODE = ORD.CUSTOMER_CODE												----# SOLVED
WHERE CUS.COUNTRY = 'France' 
	  AND DATENAME(WEEKDAY, ORDER_DATE) = 'Monday' 
	  AND DATENAME(MONTH, ORD.ORDER_DATE) = 'April' 
	  AND DATENAME(year,ORD.ORDER_DATE) = 1998
...
CREATE VIEW MIGHTY_CUSTOMER AS   ---CREATED A VIEW
SELECT CUS.CUSTOMER_CODE, 
	   CUS.COMPANY, 
	   CUS.PHONE, ORD_D.*															----# SOLVED 
FROM CUSTOMERS AS CUS
FULL OUTER JOIN ORDERS AS ORD
ON CUS.CUSTOMER_CODE = ORD.CUSTOMER_CODE
JOIN ORDER_DETAILS AS ORD_D
ON ORD.ORDER_NUMBER = ORD_D.ORDER_NUMBER

SELECT DISTINCT CUSTOMER_CODE 
FROM MIGHTY_CUSTOMER --- QUERY THE VIEW
GROUP by CUSTOMER_CODE 
HAVING COUNT(distinct PRODUCT_REF) >= 50
...
WHERE CUSTOMERS.COUNTRY = 'FRANCE'																		---# SOLVED
...
--::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
SELECT 
	   DISTINCT (SELECT COUNT(YEAR(ORDER_DATE)) FROM ORDERS WHERE YEAR(ORDER_DATE) = 1996) AS Orders_in_1996,  
	   (SELECT COUNT(YEAR(ORDER_DATE)) 
	    FROM ORDERS 
		WHERE YEAR(ORDER_DATE) = 1997) AS Orders_in_1997,																				---# SOLVED
	   ABS((SELECT COUNT(YEAR(ORDER_DATE)) FROM ORDERS WHERE YEAR(ORDER_DATE) = 1996) - 
	   (SELECT COUNT(YEAR(ORDER_DATE)) FROM ORDERS WHERE YEAR(ORDER_DATE) = 1997)) AS Difference
FROM ORDERS
